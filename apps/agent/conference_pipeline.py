"""Conference Intelligence Pipeline — full autonomous agent.

This is the top-level orchestrator for the *before-meet* conference flow.
Given a conference URL (or a pre-populated attendee list), it:

  1. **Scrape** — Uses Playwright to extract attendees from the conference
     directory page.
...
Usage (standalone / CLI demo)::
    ...
    import asyncio
    from warmth.apps.agent.conference_pipeline import ConferencePipeline
...
Usage (inside Cursor SDK agent loop with live MCP tools)::
    ...
    pipeline = ConferencePipeline(mcp_caller=agent.call_mcp_tool)
...
class ConferencePipeline:
    """Autonomous conference intelligence agent.
    ...
    async def run(
        ...
        """Run the full conference pipeline end-to-end.

        Args:
            conference_name:          Human-readable name of the conference.
        ...
        """
        print(f"\n{'='*60}")
        print(f"  CONFERENCE PIPELINE: {conference_name}")
        print(f"{'='*60}\n")
        ...
        if self.zero_bridge:
            zero_contact_ids = await self.zero_bridge.push_hot_leads(
                leads_dicts, conference_name=conference_name
            )
            zero_result = {
    ...
    async def _scrape(
        ...
        if not directory_url:
            return []
        try:
            scraper = ConferenceDirectoryScraper(headless=self.headless)
        ...
        for lead in leads:
            ...
            try:
                result = await self.calendar_client.create_event(
                    ...
                    description=(
                        f"Pre-conference intro meeting arranged by Warmth.\n\n"
